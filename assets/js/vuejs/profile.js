new Vue({
  ...
  computed: {
    ...
    searchReviewer: function () {
      return this.profileUser.voted_by.filter(item => {
        if (!this.search) 
          return this.profileUser.voted_by;
        return (item.voter.user_name.toLowerCase().includes(this.search.toLowerCase()) || item.voter.email.toLowerCase().includes(this.search.toLowerCase()) || item.voter.members.first_name.toLowerCase().includes(this.search.toLowerCase()) || item.voter.members.mid_name.toLowerCase().includes(this.search.toLowerCase()) || item.voter.members.last_name.toLowerCase().includes(this.search.toLowerCase()));
      });
    }
  },
  mounted() {
  ...
  methods: {
    ...
    updateProfile: function () {
      ...
      axios.put(url, payload, header).then(res => {
        // console.log(res)
        this.isLoading = false;
        if (res.status === 200) {
          this.generalErrorMessage = res.data.message;
    ...
    defineBedge: function () {
      ...
      for (let i = 0; i < maxLengtReff; i++) {
        ...
        if (data.id === 1) {
          ...
          bedges = totalVoting >= start && totalVoting <= end
            ? data
            : [];
          bedges.start = start;
          bedges.end = end;
    ...
    nextRecomendation: function (type) {
      ...
      axios.get(url, header).then(res => {
        ...
        if (type === 0) {
          data.map(el => {
            this.cafeRecomendation.push(el);
          });
          this.nextCafeRecomendation = res.data.data.next_page_url;
        }
        ...
        if (type === 1) {
          data.map(el => {
            this.foodRecomendation.push(el);
          });
          this.nextFoodRecomendation = res.data.data.next_page_url;
        }
        ...
        if (type === 2) {
          data.map(el => {
            this.beveragesRecomendation.push(el);
          });
          this.nextBeveragesRecomendation = res.data.data.next_page_url;
        }
      }).catch(err => {
    ...
    checkNotif: async function (target, id) {
      ...
      if (update.status === 200) {
        ...
        if (route === "mycafe") {
          ...
          if (type === "1" || type === 1) {
            // only open mycafe, and open modal food with id
            localStorage.setItem("pleaseOpen", "mycafe::1::" + id);
            window.location.replace("/mycafe");
          }

          if (type === "2" || type === 2) {
            // only open mycafe, and open modal baverages with id
    ...
    setMyLocation: function () {
      ...
      this.map.setZoom(this.map.getZoom() + 5, true);
      let prox = this.mylocation.lat + "," + this.mylocation.lng;
      this.getLocationAddress(prox);
    },
    setMarker: function () {
      if (this.map.getObjects()) {
        this.map.removeObjects(this.map.getObjects());
    ...
    getLocationCoordinate: function (searchLocation) {
      const that = this;
      ...
      geocoder.geocode(geocodingParameters, function (res) {
        if (res.response.view.length > 0) {
          let coordinate = res.response.view[0].result[0].location.displayPosition;
          that.mylocation.lat = coordinate.latitude;
          that.mylocation.lng = coordinate.longitude;
          that.setMyLocation();
        } else {
    ...
    getLocationAddress: function (proxy) {
      const that = this;
    ...
    getGeolocation: function () {
      ...
      if (!("geolocation" in navigator)) {
        this.openSnackBar("Geolocation is not available.");
        return;
      }
      ...
      navigator.geolocation.getCurrentPosition(res => {
        ...
        //         this.openSnackBar('User denied the request for Geolocation.')
        //     break;
        //     case err.POSITION_UNAVAILABLE:
        //         this.openSnackBar('Location information is unavailable.')
        //     break;
        //     case err.TIMEOUT:
        //         this.openSnackBar('The request to get user location timed out.')
        //     break;
    ...
    registerCafe: function () {
      $("#registerCafe").modal("show");
      const that = this;
      setTimeout(function () {
        if (!that.isInitialize1) {
          that.initializeMap();
          that.isInitialize1 = true;
        }
      }, 1000);
    },
    registerCafe2: function () {
      $("#registerCafe2").modal("show");
      const that = this;
      setTimeout(function () {
        if (!that.isInitialize2) {
          that.initializeMap2();
