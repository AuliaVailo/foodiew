  ...
  methods: {
    ...
    seeMoreFood: function (type) {
      ...
      axios.get(url, header).then(res => {
        ...
        if (type === 1) {
          data.map(el => {
            this.trending_foods.data.push(el);
          });
          this.nextFoods = res.data.data.next_page_url;
          localStorage.setItem("trending_foods", JSON.stringify(this.trending_foods));
        }
        ...
        if (type === 2) {
          data.map(el => {
            this.trending_baverages.data.push(el);
          });
    ...
    doDelete(id) {
      ...
      axios.delete(url, header, payload).then(res => {
        // console.log(res)
        this.isLoading = false;
        if (res.status === 200) {
