new Vue({
    ...
    methods: {
        ...
        getRecomandationCafe: async function() {
            console.log('location', this.location)
            let url = this.url + '/api/caffes/recomendation/halloffame/' + this.location.city
            // let token = 'Bearer ' + localStorage.getItem('token')
            let header = {
                // headers: {
                //     'Authorization': `${token}`,
        ...
        getRecomandationFood: function() {
            let url = this.url + '/api/foods/recomendation/1/halloffame/' + this.location.city
            // let token = 'Bearer ' + localStorage.getItem('token')
            let header = {
                // headers: {
                //     'Authorization': `${token}`,
        ...
        getRecomandationBeverages: function() {
            let url = this.url + '/api/foods/recomendation/2/halloffame/' + this.location.city
            // let token = 'Bearer ' + localStorage.getItem('token')
            let header = {
                // headers: {
                //     'Authorization': `${token}`,
                    ...
                    if (res.status === 200) {
                        ...
                        if (this.detailDialog.type === '1') { // food
                            if (this.detailDialog.promo === '1') { // food
                                ...
                                console.log(this.foodRecomendation[objIndex])
                                this.foodRecomendation[objIndex].reviews.push(res.data.data)
                            }
                        }

                        if (this.detailDialog.type === '2') { // food
                            if (this.detailDialog.promo === '1') { // food
                                let objIndex = this.promos.findIndex((obj => obj.id === this.detailDialog.id))
                                this.promos[objIndex].details.reviews.push(res.data.data)
                            } else {
                                let objIndex = this.beveragesRecomendation.findIndex((obj => obj.id === this.detailDialog.id))
                                this.beveragesRecomendation[objIndex].reviews.push(res.data.data)
                            }
                        }
